...
namespace QuantBook.Models.Yahoo
{
    public static class YahooHelper
    {
        ...
        public static string IdToTicker(int symbolId)
        {
            ...
            using (MyDbEntities db = new MyDbEntities())
            {
                IQueryable<string> query = from s in db.Symbols
        ...
        {
            BindableCollection<Symbol> res = new BindableCollection<Symbol>();
            using (MyDbEntities db = new MyDbEntities())
            {
                try
                {
                    IOrderedQueryable<Symbol> query = from s in db.Symbols orderby s.Ticker select s;
                    res.AddRange(query);
                }
                catch { }
            }
