    ...
    public class GoogleHelper
    {
        ...
        public static void GetGoogleStockQuote(BindableCollection<GoogleQuote> quotes)
        {
            ...
            if (!string.IsNullOrEmpty(json))
            {
                ...
                foreach (XElement item in results.Elements("item"))
                {
                    try
                    {
                        IEnumerable<GoogleQuote> quote = from q in quotes where q.Symbol == item.Element("t").Value select q;
                        
                        foreach(GoogleQuote p in quote)
                        {
