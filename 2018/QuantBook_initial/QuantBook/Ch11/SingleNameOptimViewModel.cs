...
namespace QuantBook.Ch11
{
    ...
    public class SingleNameOptimViewModel : Screen
    {
        ...
        private DataTable optimTable = null;
        ...
        public async void StartOptim()
        {
            await Task.Run(() =>
            {
                ...
                DataTable dt = OptimHelper.OptimSingleName(data, SelectedSignalType, SelectedStrategyType, IsReinvest, _events);
                OptimTable = dt;
            });
        }
        ...
        public async void SelectedCellChanged(object sender, SelectedCellsChangedEventArgs e)
        {
            await Task.Run(() =>
            {
                try
                {
                    ...
                    double signalIn = dvr[2].To<double>();
                    double signalOut = dvr[3].To<double>();
                    BindableCollection<PnLEntity> pnl = BacktestHelper.ComputeLongShortPnL(signal, 10000.0, signalIn, signalOut, SelectedStrategyType, IsReinvest);
                    PnLCollection.Clear();
                    PnLCollection.AddRange(pnl);
                    DataTable dt = BacktestHelper.GetYearlyPnL(PnLCollection);
                    YearlyPnLTable = dt;
                    drawdownTable = BacktestHelper.GetDrawDown(PnLCollection, 10000.0);
                    AddPnLCharts();
                }
                catch { }
            });
