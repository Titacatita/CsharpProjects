...
namespace QuantBook.Ch08
{
    ...
    public class NNClassificationViewModel : Screen
    {
        ...
        private double[] NnClassification3(DataTable dtData)
        {
            ...
            BipolarSigmoidFunction function = new BipolarSigmoidFunction(Alpha);
            ActivationNetwork nn = new ActivationNetwork(function, 4, HiddenNeurons, 3);

            NguyenWidrow win = new NguyenWidrow(nn);
            win.Randomize();

            LevenbergMarquardtLearning lm = new LevenbergMarquardtLearning(nn);
            lm.LearningRate = LearningRate;
            ParallelResilientBackpropagationLearning rprop = new ParallelResilientBackpropagationLearning(nn);
        ...
        private double[] NnClassification2(DataTable dtData1)
        {
            ...
            ActivationNetwork nn = new ActivationNetwork(new BipolarSigmoidFunction(Alpha), 4, HiddenNeurons, 1);
            NguyenWidrow win = new NguyenWidrow(nn);
            win.Randomize();
