...
namespace QuantBook.Models.Google
{
    public static class GoogleOptions
    {
        ...
        public static BindableCollection<OptionExpiration> GetExpiries(XElement element)
        {
            ...
            foreach (XElement item in element.Element("expirations").Elements("item"))
            {
        ...
        public static BindableCollection<OptionResults> GetOptionResults(XElement element, string ticker)
        {
            ...
            foreach(XElement item in calls.Elements("item"))
            {
            ...
            foreach(XElement item in puts.Elements("item"))
            {
                decimal strike = item.Element("strike").Value.To<decimal>();
                System.Collections.Generic.IEnumerable<OptionResults> cc = from c in res where c.Strike == strike select c;
                if(cc.Count()>0)
                {
                    foreach(OptionResults r in cc)
                    {
