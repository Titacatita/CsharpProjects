...
namespace QuantBook.Models.DataModel
{
    public static class Dal
    {
        ...
        public static BindableCollection<StockData> GetStockDataFromGoogle(string ticker, DateTime startDate, DateTime endDate)
        {
            ...
            if (dt.Count > 0)
            {
                foreach (GoogleStockPrice p in dt)
                {
                    res.Add(new StockData
                    {
                        Ticker = ticker,
        ...
        {
            BindableCollection<StockData> res = new BindableCollection<StockData>();
            DataTable dt = ModelHelper.CsvToDatatable(fileName);
        ...
        {
            BindableCollection<IndexData> res = new BindableCollection<IndexData>();
            BindableCollection<IndexData> data = GetIndexData();
            System.Collections.Generic.IEnumerable<IndexData> query = from p in data where p.Date >= startDate && p.Date <= endDate select p;
            foreach (IndexData p in query)
                res.Add(p);
            return res;
        }
        ...
        public static DataTable GetMultiStockData(string[] tickers, DateTime startDate, DateTime endDate, string priceType, DataSourceEnum dataSource)
        {
            ...
            {
                BindableCollection<StockData> tmp =  GetStockData(ticker, startDate, endDate, dataSource);
                if(tmp.Count<1)
                {
            ...
            while (date <= endDate)
            {
                IOrderedEnumerable<StockData> dts = from p in dt where p.Date == date orderby p.Ticker select p;
                if (dts.Count() == tickers.Length)
                {
                    if (count == 0)
                    {
                        res.Columns.Add("Date", typeof(DateTime));
                        foreach (StockData r in dts)
